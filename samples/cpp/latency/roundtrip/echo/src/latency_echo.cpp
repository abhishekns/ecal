...
eCAL::CPublisher* getPublisher(int snd_size /*kB*/, int mem_buffer, bool zero_copy) { 
  ...
  auto pubItr = publishers.find(snd_size);
  if (pubItr == publishers.end()) {
    eCAL::CPublisher* pub = new eCAL::CPublisher("roundtrip-fro");
    ...
    publishers[snd_size] = pub;
    return pub;
  }
  return pubItr->second;
}

// single test run
void Send(const struct eCAL::SReceiveCallbackData* data_, int mem_buffer = 1, bool zero_copy = false)
{
  // let them match
  auto pub = getPublisher(data_->size, mem_buffer, zero_copy);

  pub->Send(data_->buf, data_->size, data_->time);
...
{
  // initialize eCAL API
  eCAL::Initialize(0, nullptr, "latency_echo");

  // subscriber
  eCAL::CSubscriber sub("roundtrip-to");
